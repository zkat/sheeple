...
** Built-in
   ...
   be fetched using e.g. (proto 'string).
...
*** DEFPROTO macro
    ...
    table where it can be accessed by using (proto 'name). After the new object has been
...
*** PROTO
    Syntax:
    proto name &optional (errorp t) => sheep

    Args and values:
    name -- a non-keyword symbol denoting the name of a proto.
    sheep -- a proto
    errorp -- whether an error should be signaled if there is no such proto.

    Description:
    Looks up a proto sheep with name NAME in the global protos table. If errorp is T,
    an ERROR is signaled if the lookup fails, otherwise, NIL is returned.

    Examples:
    
      (defproto test-proto () ()) => #<Sheep AKA: TEST-PROTO #x14F31536>
      (proto 'test-proto) => #<Sheep AKA: TEST-PROTO #x14F31536>
...
**** Specialized replies
     Replies are specialized by replacing the variable with (specialized-variable object).
        ...
        (defreply reply-name (foo (bar (proto 'my-sheep)))
...
**** Huge example
     This example illustrates usage of sheeple and replies:
        ...
        (defreply do-it (foo) ;defines this reply on (proto 'T)
          (print foo))
        (do-it "hey!") => hey! ;the reply facility autoboxes regular lisp objects

        (defreply do-it ((something (proto 'sheep1))) ;a convenient way of saying (find-proto 'sheep1)
        ...
        (do-it (proto 'sheep1)) => #<Sheep AKA: SHEEP1 {BA28B89}> is the first sheep. FYI

        (let ((x 5))
	   (defreply do-it ((foo (proto 'number))) ;if dispatching on builtins, use builtin prototype.
	  ...
	  (print "FIVE!! THIS IS VERY EXCITING!!1"))
        ...
        (defreply do-it ((foo (proto 'float)))
          (floatp float))
        ...
        (defreply synergize ((foo (proto 'sheep1)) (bar (proto 'sheep2)))
          (print "I got sheep1 first, then sheep2"))

        (defreply synergize ((foo (proto 'sheep2)) (bar (proto 'sheep1)))
          (declare (ignore foo bar))
          (print "I got sheep2 first, then the first sheep."))

        (defreply synergize ((foo (proto 'number)) (bar (proto 'number)))
          (+ foo bar))
        (synergize 3 5) => 8
        (synergize 4 "hey hey hey!") => ERROR: NO-MOST-SPECIFIC-REPLY
        
        (defreply synergize ((foo (proto 'string)) (bar (proto 'string)))
...
*** CLOS (semi-)compatibility
    ...
    (proto 'builtin), not as autoboxed versions of the classes/objects.
...
**** SHEEP-PARENTS
     ...
     (setf parent (clone)
           child1 (clone parent)
           child2 (clone parent (proto 'dolly)))
