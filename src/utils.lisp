...
(defmacro define-print-object ((class &key (identity t) (type t)) &body body)
  (multiple-value-bind (object-var class-var)
      (cond ((atom class) (values class class))
            ((= (length class) 2) (values-list class))
            (t (error "~A is not an atom or a list (OBJECT CLASS)" class)))
    (with-gensyms (stream)
      `(defmethod print-object ((,object-var ,class-var) ,stream)
