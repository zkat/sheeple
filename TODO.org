...
** TODO [#B] Cache message dispatch
   ...
   but it should definitely be implemented and *ready* by the time 3.1 is tagged -- sykopomp
* TODO 3.0
...
** DONE Instantiation
   ...
   defsheep, in the end.
   UPDATE: I think the interface is fine for now, thinking about it. --sykopomp
** DONE Properties
...
*** DONE Improve the whole add-property interface.
    ...
    defproperty macro?
    UPDATE: I think the new interface for adding properties is pretty acceptable --sykopomp
...
*** DONE Write proper print functions
    ...
    messing around with #'print-object.
*** TODO #'std-remove-parent should make sure that the hierarchy list is still valid.
*** DONE Once bootstrapped, update WITH-SHEEP-HIERARCHY to use SPAWN
