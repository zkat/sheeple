...
(test (std-sheep-accessors
       :depends-on std-sheep-basic-structure
       :fixture allocate-std-sheep)
  (is (eq (setf (car sheep) (gensym)) (std-sheep-metasheep sheep)))
  (is (eq (setf (svref (cdr sheep) 0) (gensym)) (std-sheep-parents sheep)))
  (is (eq (setf (svref (cdr sheep) 1) (gensym)) (std-sheep-pvalue-vector sheep)))
  (is (eq (setf (svref (cdr sheep) 2) (gensym)) (std-sheep-property-metasheep sheep)))
  (is (eq (setf (svref (cdr sheep) 3) (gensym)) (std-sheep-roles sheep)))
  (is (eq (setf (svref (cdr sheep) 4) (gensym)) (std-sheep-hierarchy-cache sheep)))
  (is (eq (setf (svref (cdr sheep) 5) (gensym)) (std-sheep-children sheep))))
...
(test (std-sheep-p :fixture allocate-std-sheep)
  ...
  (setf (std-sheep-parents sheep) '(foo))
  (is (std-sheep-p sheep)))
...
(test (std-sheep-metasheep :fixture allocate-std-sheep)
  (is (eql =standard-metasheep= (std-sheep-metasheep sheep))))

(test (std-sheep-parents :fixture allocate-std-sheep)
  (is (eql nil (std-sheep-parents sheep)))
  (is (equal '(foo) (setf (std-sheep-parents sheep) '(foo))))
  (is (equal '(foo) (std-sheep-parents sheep)))
  (is (equal '(bar foo) (push 'bar (std-sheep-parents sheep))))
  (is (equal '(bar foo) (std-sheep-parents sheep))))

(test (std-sheep-pvalue-vector :fixture allocate-std-sheep)
  (is (eql nil (std-sheep-pvalue-vector sheep)))
  (is (equal '(foo) (setf (std-sheep-pvalue-vector sheep) '(foo))))
  (is (equal '(foo) (std-sheep-pvalue-vector sheep)))
  (is (equal '(bar foo) (push 'bar (std-sheep-pvalue-vector sheep))))
  (is (equal '(bar foo) (std-sheep-pvalue-vector sheep))))

(test (std-sheep-property-metasheep :fixture allocate-std-sheep)
  (is (eql nil (std-sheep-property-metasheep sheep)))
  (is (equal '(foo) (setf (std-sheep-property-metasheep sheep) '(foo))))
  (is (equal '(foo) (std-sheep-property-metasheep sheep)))
  (is (equal '(bar foo) (push 'bar (std-sheep-property-metasheep sheep))))
  (is (equal '(bar foo) (std-sheep-property-metasheep sheep))))

(test (std-sheep-roles :fixture allocate-std-sheep)
  (is (eql nil (std-sheep-roles sheep)))
  (is (equal '(foo) (setf (std-sheep-roles sheep) '(foo))))
  (is (equal '(foo) (std-sheep-roles sheep)))
  (is (equal '(bar foo) (push 'bar (std-sheep-roles sheep))))
  (is (equal '(bar foo) (std-sheep-roles sheep))))

(test (std-sheep-hierarchy-cache :fixture allocate-std-sheep)
  (is (eql nil (std-sheep-hierarchy-cache sheep)))
  (is (equal '(foo) (setf (std-sheep-hierarchy-cache sheep) '(foo))))
  (is (equal '(foo) (std-sheep-hierarchy-cache sheep)))
  (is (equal '(bar foo) (push 'bar (std-sheep-hierarchy-cache sheep))))
  (is (equal '(bar foo) (std-sheep-hierarchy-cache sheep))))

(test (std-sheep-children :fixture allocate-std-sheep)
  (is (eql nil (std-sheep-children sheep)))
  (is (equal '(foo) (setf (std-sheep-children sheep) '(foo))))
  (is (equal '(foo) (std-sheep-children sheep)))
  (is (equal '(bar foo) (push 'bar (std-sheep-children sheep))))
  (is (equal '(bar foo) (std-sheep-children sheep))))
...
(test collect-ancestors
  (let ((sheep1 (allocate-std-sheep))
    ...
    (setf (std-sheep-parents sheep1) (list sheep2))
    (setf (std-sheep-parents sheep2) (list sheep3))
...
(test local-precedence-ordering
  (let* ((a (allocate-std-sheep))
    ...
    (setf (std-sheep-parents a) (list =standard-sheep=))
    (setf (std-sheep-parents b) (list =standard-sheep=))
    (setf (std-sheep-parents c) (list =standard-sheep=))
    (setf (std-sheep-parents d) (list a b c))
...
(test std-tie-breaker-rule
  (let* ((a (allocate-std-sheep))
    ...
    (setf (std-sheep-parents b) (list =standard-sheep=))
    (setf (std-sheep-parents c) (list =standard-sheep=))
    (setf (std-sheep-parents e) (list a))
    (setf (std-sheep-parents f) (list b))
    (setf (std-sheep-parents g) (list c))
...
(test compute-sheep-hierarchy-list
  (let* ((parent (allocate-std-sheep))
    ...
    (setf (std-sheep-parents child) (list parent))
  ...
  (let* ((a (allocate-std-sheep))
    ...
    (setf (std-sheep-parents c) (list a))
    (setf (std-sheep-parents d) (list a))
    (setf (std-sheep-parents e) (list b c))
    (setf (std-sheep-parents f) (list d))
    (setf (std-sheep-parents g) (list c f))
    (setf (std-sheep-parents h) (list g e))
...
(test (child-cache-full-p :fixture allocate-std-sheep)
  ...
  (setf (std-sheep-children sheep)
        (make-array 5 :initial-element nil))
  (is (not (child-cache-full-p sheep)))
  (setf (aref (std-sheep-children sheep)
              1)
...
(test (adjust-child-cache :fixture allocate-std-sheep)
  ...
  (is (= 5 (length (std-sheep-children sheep))))
  (is (not (adjustable-array-p (std-sheep-children sheep))))
  (setf (aref (std-sheep-children sheep) 0) (make-weak-pointer sheep))
  (setf (aref (std-sheep-children sheep) 1) (make-weak-pointer sheep))
  (setf (aref (std-sheep-children sheep) 2) (make-weak-pointer sheep))
  (setf (aref (std-sheep-children sheep) 3) (make-weak-pointer sheep))
  (setf (aref (std-sheep-children sheep) 4) (make-weak-pointer sheep))
  (is (eql sheep (adjust-child-cache sheep)))
  (is (= 100 (length (std-sheep-children sheep))))
  (is (eql sheep (weak-pointer-value (aref (std-sheep-children sheep) 0))))
  (is (eql sheep (weak-pointer-value (aref (std-sheep-children sheep) 1))))
  (is (eql sheep (weak-pointer-value (aref (std-sheep-children sheep) 2))))
  (is (eql sheep (weak-pointer-value (aref (std-sheep-children sheep) 3))))
  (is (eql sheep (weak-pointer-value (aref (std-sheep-children sheep) 4))))
  (is (adjustable-array-p (std-sheep-children sheep)))
  (is (eql sheep (adjust-child-cache sheep)))
  (is (= 200 (length (std-sheep-children sheep))))
  (is (eql sheep (weak-pointer-value (aref (std-sheep-children sheep) 0))))
  (is (eql sheep (weak-pointer-value (aref (std-sheep-children sheep) 1))))
  (is (eql sheep (weak-pointer-value (aref (std-sheep-children sheep) 2))))
  (is (eql sheep (weak-pointer-value (aref (std-sheep-children sheep) 3))))
...
(test (create-child-cache :fixture allocate-std-sheep)
  (is (null (std-sheep-children sheep)))
  (is (simple-vector-p (create-child-cache sheep)))
  (is (simple-vector-p (std-sheep-children sheep))))

(test add-child
  (let ((sheep1 (allocate-std-sheep))
        (sheep2 (allocate-std-sheep)))
    (is (null (std-sheep-children sheep1)))
    (is (null (std-sheep-children sheep2)))
    (is (eql sheep1 (add-child sheep2 sheep1)))
    (is (null (std-sheep-children sheep2)))
    (is (simple-vector-p (std-sheep-children sheep1)))
    (is (find sheep2 (std-sheep-children sheep1) :key #'maybe-weak-pointer-value))))
...
(test (overwriting-garbage-children :depends-on add-child)
  (loop :repeat 5 :for sheep := (allocate-std-sheep)
     :do (loop :repeat 5 :do (add-child (allocate-std-sheep) sheep)
            ...
            :finally (gc :full t))
     (unless (every #'maybe-weak-pointer-value (std-sheep-children sheep))
       (return
         (if (= 5 (length (std-sheep-children (add-child (allocate-std-sheep) sheep))))
...
(test remove-child
  (let ((sheep1 (allocate-std-sheep))
    ...
    (is (simple-vector-p (std-sheep-children sheep1)))
    (is (not (find sheep2 (std-sheep-children sheep1) :key #'maybe-weak-pointer-value)))))
...
(test memoize-sheep-hierarchy-list
  (let ((sheep1 (allocate-std-sheep))
        (sheep2 (allocate-std-sheep)))
    (setf (std-sheep-parents sheep1) (list sheep2))
    (is (null (std-sheep-hierarchy-cache sheep1)))
    (is (null (std-sheep-hierarchy-cache sheep2)))
    (memoize-sheep-hierarchy-list sheep1)
    (is (equal (list sheep1 sheep2) (std-sheep-hierarchy-cache sheep1)))))

(test std-finalize-sheep-inheritance
  (let ((sheep1 (allocate-std-sheep))
        (sheep2 (allocate-std-sheep)))
    (is (equal (list sheep2) (setf (std-sheep-parents sheep1) (list sheep2))))
    (is (eql sheep1 (std-finalize-sheep-inheritance sheep1)))
    (is (find sheep2 (std-sheep-parents sheep1)))
    (is (find sheep2 (std-sheep-hierarchy-cache sheep1)))))
...
(test add-parents
  (let ((a (allocate-std-sheep))
    ...
    (is (equal (list a b) (std-sheep-parents c)))))
